...
export const COUNCIL_MCP_SERVER_VERSION = "1.3.0";
...
export async function runCouncilMcpServer(options: { storePath?: string; store?: CouncilStore; wakeDelivery?: CouncilWakeDelivery; managedRuntime?: CouncilManagedRuntime; observations?: CouncilObservationStore }): Promise<void> {
  ...
  await createCouncilMcpServer(store, { wakeDelivery: options.wakeDelivery, managedRuntime: options.managedRuntime, observations: options.observations }).connect(new StdioServerTransport());
}
