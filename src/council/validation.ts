...
export function emptyCouncilState(): CouncilState { return { version: 1, agents: [], credentials: [], rooms: [], messages: [], decisions: [], tasks: [], wakes: [], checkpoints: [] }; }
export function assertCouncilState(value: unknown): CouncilState {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Council state is not an object");
  const state = value as Partial<CouncilState>;
  if (state.version !== 1) throw new Error("Council state version is unsupported");
  // Backward-compatible migration for pre-capability V1 state. Existing agents become one-time
  // claimable: the first successful council_join after upgrade receives their new capability.
  if (!Array.isArray(state.credentials)) state.credentials = [];
  for (const key of ["agents", "credentials", "rooms", "messages", "decisions", "tasks", "wakes", "checkpoints"] as const) {
    if (!Array.isArray(state[key])) throw new Error(`Council state is missing ${key}`);
  }
  return state as CouncilState;
}
