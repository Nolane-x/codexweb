...
export function buildCouncilWakePrompt(packet: CouncilContextPacket): string {
  ...
  return ["<codexweb_council_wake>", "You are being resumed as a named participant in a local ChatGPT Council.", "This is a continuity packet, not a request to reveal private chain-of-thought.", "", `IDENTITY: ${packet.identity.id} (${packet.identity.name}) — ${packet.identity.role}`, `ROOM: ${packet.room.id} — ${packet.room.name}`, `MISSION: ${packet.room.mission}`, `WAKE REASON: ${packet.wake?.reason ?? "Council continuation"}`, "", "Required protocol:", `1. Call council_join with agent_id=${identity}, name=${JSON.stringify(packet.identity.name)}, role=${JSON.stringify(packet.identity.role)}.`, `2. On EVERY later Council tool call, pass agent_id=${identity}. Never use another participant's id.`, `3. Call council_context with agent_id=${identity}, room_id=${room} and read the latest shared state before deciding what to do.`, `4. Read/reply to relevant Council messages using agent_id=${identity}. Do not merely answer this transport prompt; record useful discussion in the Council room with council_say/council_reply.`, `5. If deliberation is mature and your role permits it, record a decision and create/assign concrete tasks using agent_id=${identity}. Otherwise challenge assumptions, add evidence, or request another participant with council_wake.`, `6. Save a compact council_checkpoint with agent_id=${identity} before finishing so a later wake can restore you quickly.`, "7. Do not expose credentials, connector internals, hidden reasoning, or chain-of-thought. Share conclusions, evidence, objections, decisions, and work state only.", "", "BOOTSTRAP CONTEXT PACKET:", JSON.stringify(packet, null, 2), "</codexweb_council_wake>"].join("\n");
}
