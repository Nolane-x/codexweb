...
const REPOSITORY = "Nolane-x/codexweb";
const RELEASE_API_URL = `https://api.github.com/repos/${REPOSITORY}/releases/latest`;
const USER_AGENT = "codexweb-council-launcher-updater";
...
function releaseAssetName(version, platform = process.platform, arch = process.arch) {
  if (platform === "darwin" && ["arm64", "x64"].includes(arch)) return `codex-web-gpt-${version}-mac-${arch}.zip`;
  if (platform === "win32" && arch === "x64") return `codex-web-gpt-${version}-win-x64.exe`;
  if (platform === "linux" && arch === "x64") return `codex-web-gpt-${version}-linux-x64.AppImage`;
  return null;
}
...
function findMacApplication(root) {
  ...
  if (!fs.existsSync(executable) || !fs.statSync(executable).isFile()) throw new Error("The macOS update archive is incomplete");
  return application;
}

function buildJob({ version, platform, executablePath, assetPath, stagingRoot, tempRoot, logPath }) {
  if (platform === "darwin") return { version, platform, parentPid: process.pid, tempRoot, logPath, source: findMacApplication(stagingRoot), target: macApplicationPath(executablePath) };
  if (platform === "win32") return { version, platform, parentPid: process.pid, tempRoot, logPath, source: assetPath, target: executablePath };
  if (platform === "linux") {
    const target = process.env.CODEX_WEB_GPT_APPIMAGE?.trim() || process.env.APPIMAGE?.trim();
    if (!target || !path.isAbsolute(target)) throw new Error("The running Linux AppImage path is unavailable; reinstall with install-launcher.sh");
    return { version, platform, parentPid: process.pid, tempRoot, logPath, source: assetPath, target, wrapper: process.env.CODEX_WEB_GPT_LAUNCHER_EXECUTABLE?.trim() || null };
  }
...
function defaultDependencies() {
  return {
    ...
    extractMac(archive, destination) {
      ...
      const result = spawnSync("/usr/bin/ditto", ["-x", "-k", archive, destination], { encoding: "utf8", timeout: 120_000 });
...
function createUpdateController({ currentVersion, platform, arch, packaged, executablePath, runtimeExecutable, logsDirectory, publish, logger, dependencies = {} }) {
  ...
  async function beginInstall() {
    ...
    pending = (async () => {
      ...
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "codexweb-council-update-"));
      try {
        ...
        const job = buildJob({ version: available.version, platform, executablePath, assetPath, stagingRoot, tempRoot, logPath: path.join(logsDirectory, "update-worker.log") });
...
module.exports = { REPOSITORY, buildJob, compareVersions, createUpdateController, expectedChecksum, macApplicationPath, parseVersion, releaseAssetName, releaseVersion, validateReleaseAssetUrl };
